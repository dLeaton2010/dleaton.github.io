...
class TemperatureMachine(StateMachine):
    ...
    ##
    ## Buffer range used to reduce short cycling around the set point
    ##
    buffer = 0.5

    ##
    ## Store the previous temperature reading for predictive control
    ##
    previousTemp = None
    ...
    def updateLights(self):
        ...
        ## Use predictive temperature control instead of only reacting to current temperature
        predictedTemp = self.getPredictedTemperature()

        ## Round the predicted temperature for easier comparison and display
        temp = predictedTemp
        ...
        if(DEBUG):
            ...
            print(f"Predicted Temp: {temp:.2f}")

        # Determine visual identifiers using predictive control and a buffer range
        if(self.current_state.id == 'heat'):
            # Thermostat is set to heat
            if temp < (self.setPoint - self.buffer):
                # Predicted temperature is below the lower buffer threshold
                redLight.pulse()
            else:
                # Predicted temperature is within range or above the threshold
                redLight.on()

        elif(self.current_state.id == 'cool'):
            # Thermostat is set to cool
            if temp > (self.setPoint + self.buffer):
                # Predicted temperature is above the upper buffer threshold
                blueLight.pulse()
            else:
                # Predicted temperature is within range or below the threshold
                blueLight.on()
    ...
    ##
    ## Predict the next temperature based on the current and previous readings
    ##
    def getPredictedTemperature(self):
        currentTemp = self.getFahrenheit()

        # If there is no previous reading yet, use the current temperature
        if self.previousTemp is None:
            self.previousTemp = currentTemp
            return currentTemp

        # Calculate temperature trend
        rateOfChange = currentTemp - self.previousTemp
        predictedTemp = currentTemp + rateOfChange

        # Update previous temperature for the next prediction
        self.previousTemp = currentTemp
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                lcd_line_2 = f"T:{self.getFahrenheit(): 0.1g}F"
