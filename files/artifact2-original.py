...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        if(self.current_state.id == 'heat'):
            # thermostate is set to heat
            if (temp >= self.setPoint):
                #temp is above the set temp
                redLight.on()
            else:
                # temp is lower than current set temp
                redLight.pulse()
        elif(self.current_state.id == 'cool'):
            # thermostate is set to cool
            if(temp <= self.setPoint):
                #temp is below the set temp
                blueLight.on()
            else:
                # temp is above current set temp
                blueLight.pulse()
        else:
            # thermostat is off
            pass
